...
def process_video_frame(frame, u_map, v_map, crop_width, idx, temp_folder, blur):
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    H, W, _ = frame.shape

    frame_cropped = frame[:, :crop_width]

    frame_resized = cv2.resize(frame_cropped, (2048, 2048), interpolation=cv2.INTER_LINEAR)
    ...
    # Resize the result to a max of 2048x2048 using Lanczos filter
    H, W, _ = result.shape
    if H > 2048 or W > 2048:
        scaling_factor = min(2048 / H, 2048 / W)
...
def process_video(video_path, output_path, u_map, v_map, split, num_workers, temp_folder, blur):
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for idx in range(length):
            ...
            if not ret:
                break
            futures[executor.submit(process_video_frame, frame, u_map, v_map, crop_width, idx, temp_folder, blur)] = idx
    ...
    create_video_from_images(temp_folder, output_path)
    
    shutil.rmtree(temp_folder)

def create_video_from_images(temp_folder, output_path):
...
if __name__ == "__main__":
    ...
    process_video(args.input_video, args.output_video, u_map, v_map, args.split, args.num_workers, args.temp_folder, args.blur)
